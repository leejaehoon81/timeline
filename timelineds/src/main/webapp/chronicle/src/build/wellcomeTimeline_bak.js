...
//-------
function julianToGregorian(jd) // parameter 값으로 juliandate전달
{
...
function roundNumber(num, dec) { // 안씀
...
function fitRect(width1, height1, width2, height2) { // 안씀
...
// querystring - timeline.html에서 파라메터값 알아낼때 씀
...
(function ($) {
    ...
    $.widget("wellcome.timeline_baseTimeline", {
        ...
        _create: function () {
            ...
            // store global reference to plugin. 플러그인에 글로벌 참조변수를 저장
            ...
            self.SHOW_EMBED_DIALOGUE = "onShowEmbedDialogue"; // embed관련 삭제함
            self.HIDE_EMBED_DIALOGUE = "onHideEmbedDialogue"; // embed관련 삭제함
            self.SHOW_HELP_DIALOGUE = "onShowHelpDialogue"; // help도 삭제함
            self.HIDE_HELP_DIALOGUE = "onHideHelpDialogue"; // help도 삭제함
            ...
            // bind to global events. 글로벌 이벤트에 바인딩하는데 왜 풀스크린???
            ...
            // add classes. embedded란 클래스 add하는건데 element에서 한번도 못봄...
            if (!self.options.isHomeDomain) self.element.addClass('embedded');

            // keyboard events. numericalInput함수 호출할때 쓰이는듯?
            $(document).keyup(function(e) {
                if (e.keyCode === 27) self._trigger(self.ESCAPE);  // ESC키 눌렀다 뗄때
                if (e.keyCode === 13) self._trigger(self.RETURN);  // Enter키 눌렀다 뗄때
            });

            // mouse position. 마우스의 x,y좌표를 알수있다.. setArrowPosition메소드 호출할때 쓰인다
            $(document).mousemove(function (e) {
                self.mouseX = e.pageX;
                self.mouseY = e.pageY;
            });

            if (!self.options.isHomeDomain){ // 홈도메인이 아니라면..... 
                trackVariable(2, 'Timeline Embedded', self.options.url, 2);
            }
        },  // 여기까지 timeline_baseTimeline 위젯을 생성할때 자동적으로 호출됨

        changeIndex: function (index) { // 이벤트의 인덱스가 파라미터값임 인덱스이기때문에 0부터 시작 db가 10개 있으면 0 ~ 9
            var self = this;

            if (index < -1) return;
            if (index > self.provider.data.Events.length - 1) return;
            
            self.lastIndex = self.currentIndex; // 이벤트를 바꾸면 이전에 선택했던 인덱스가 마지막 인덱스로 간다
            self._deselectCurrentEvent(); // _deselectCurrentEvent()를 실행한 뒤 
            self.currentIndex = index; // 전달받은 인덱스가 현재 인덱스로 됨

            self._trigger(self.START_INDEX_CHANGE, index); // 이벤트전파하는거 아직 잘 모름 질문 ㄱㄱ
            self._trigger(self.SHOW_EVENT_DETAILS_DIALOGUE);

            var eventId = "0";

            if (index != -1) {
                // timeline위젯의 getEventByIndex메소드에 index를 전달해서 DB에 저장된 해당 이벤트를 알아내서 EventId값(DB column)을 가져온다
                eventId = $.wellcome.timeline.getEventByIndex(index).EventId;
            }
            // setAddress에 eventId를 줘서 uri를 변경
        ...
        viewEvent: function(eventId) {
            ...
            for (var i = 0, l = self.provider.data.Events.length; i < l; i++) {
                ...
                if (evnt.EventId == eventId) {
                    // give IE a bit of breathing room... 이벤트 선택할때 딜레이 0.1초...
                    setTimeout(function () {
                        self.changeIndex(i);
                    }, 100);
        ...
        getView: function (name) { // header, main, details, genericDialogue
            var self = this;
            return self.element.timeline_shell('getView', name);
        },

        // 절대 url과 상대 url.. 왜쓰나?????? 함수 호출을 안함...
        ...
        // non-destructive address update. 비파괴적인 주소 업데이트... 안씀... 
        ...
        // destructive address update. 파괴적인 주소 업데이트.... 이건 씀
        setAddress: function () {
            var self = this;
        ...
        _resize: function () { // 창 크기 변경할때 존나 씀
            var self = this;
...
// wellcomeTimeline 위젯.. db로딩하는 위젯 _create할때 _viewPackage에서 db로딩
(function ($) {
    ...
    $.widget("wellcome.wellcomeTimeline", {
        ...
        _create: function () {
            ...
            // load config.
            // self.options.configUri => js/config.js 불러옴
            $.getJSON(self.options.configUri, function (config) {
                self.config = config;

                // load data. sample_data.js 불러옴
...
// timeline 위젯... timeline_baseTimeline위젯을 상속받는다
(function ($) {
    ...
    $.widget("wellcome.timeline", $.wellcome.timeline_baseTimeline, {
        ...
        _create: function () {
            ...
            // footer panel footer함수 호출 2015.9.9
        ...
        _deselectCurrentEvent: function () { // 이벤트 선택된거 취소함
            var self = this;
...
(function ($) {
    ...
    $.widget("wellcome.timeline_shell", {
        ...
        _resize: function () {
            ...
            // position details view 초기위치 잡아줌
...
(function ($) {
    ...
    $.widget("wellcome.timeline_headerPanelView", {
        ...
        _create: function () {
            ...
            self.title = $.wellcome.timeline.options.provider.data.Title; // db의 Title을 self.title에
            ...
            // ui event handlers.
            self.zoomInButtonElem.on('click', function (e) { // div class="zoomIn"
                e.preventDefault();

                if (self.isZoomInEnabled) { // 줌 가능하면 onZoomIn 이벤트 발생
...
(function ($) {
    ...
    $.widget("wellcome.timeline_mainPanelView", {
        ...
        _init: function () {
            ...
            self.scroll = new iScroll('scroll', { // main event scroll 관련
                hScrollbar: false,
                vScrollbar: false,
                hideScrollbar: true
            });
            
            $(self.scroll.scroller).on('selectstart', function () { // onselectstart event - 개체가 선택되기 시작할 때 발생
                return false;
            });
            ...
            self.days = self.endDate.diff(self.startDate, 'days'); // diff different인듯...
            self.years = self.endDate.diff(self.startDate, 'years');
            self.decades = Math.floor(self.years / 10); // javacript 버림함수 floor
        ...
        _createEvents: function () {
            ...
            // to signify that.
            var present = moment(); // moment.js?? 현재날짜 저장됨
            ...
            for (i = 0, l = self.events.length; i < l; i++) {
                ...
                if (evnt.ThumbnailPath) { // 썸네일이 있으면 min 제거, src에 썸네일 경로
                ...
                titleElem.text(evnt.Title); // 타이틀 넣음
                titleElem.ellipsis(45); // 45자 넘으면 ...으로 찍히는듯

                if (evnt.isPresent) { // 현재 표시
                    dateElem.text(evnt.startDate.year());
                    elem.addClass('present');
                } else {
                    dateElem.text(evnt.StartDisplayYear);
                    // StartDisplayYear: title
                    elem.prop('title', evnt.StartDisplayYear + ": " + evnt.Title);
                    
                    elem.on('click', function () {
                        if (self.isNavigating || self.isZooming) return; // 클릭했을때  네이게이팅중이거나 줌하고있으면 return
                        
                        var index = parseInt($(this).data('index'));
                        self._selectEvent(index); // 해당인덱스 이벤트 선택
                    });

                    elem.on('mouseenter', function () { // 마우스오버 하이라이트
                    ...
                    elem.on('mouseleave', function () { // 마우스아웃 하이라이트 해제
                        if (self.isNavigating || self.isZooming) return;
                ...
                self.eventsElem.append(elem); // div class='events'에 event를 추가

                var dayIndex = evnt.startDate.diff(self.startDate, 'days'); // 일수로계산
                evnt.position = normalise(dayIndex, 0, self.days); // normalise(num, min, max) return (num - min) / (max - min)
                evnt.width = evnt.elem.outerWidth(); // border포함 width인듯

                evnt.elem.hide(); // jquery hide()
            }
            ...
            for (i = 0, l = self.events.length; i < l; i++) { // 이벤트 스택사이즈에따라 이벤트 추가.. 초기 스택 3으로 설정됨
                evnt = self.events[i];
        ...
        _createTicks: function () {
            ...
            // 1 year is the minimum timespan. 1800년부터 2099년까지 1년단위로 tick을 만들어놈
        ...
        _setEventsZIndex: function (stackLevel) { // 이벤트들의 z-index
            var self = this;
        ...
        _setEventZIndex: function (evnt) { // 이벤트의 z-index   (1,3,5)
            var self = this;
        ...
        _setCurrentEventToTop: function () { // 함수호출 안됨
            var self = this;
        ...
        _updateStackLevelVisibleEvents: function (stackLevel) { // priority
...
// 2015.9.9 하단의 footer panel 관련 (embed와 toggle fullscreen button)
/*(function ($) {
...
// 2015.9.10 embed관련 ui삭제 


// 2015.9.9 하단의 detail ui 관련
(function ($) {
    ...
    $.widget("wellcome.timeline_detailsView", $.wellcome.timeline_baseDialogueView, {
        ...
        _resize: function() {
            ...
            self.centerColElem.find('.wrapper').width(centerColWidth); // wrapper의 width를 위에서 계산한걸로
